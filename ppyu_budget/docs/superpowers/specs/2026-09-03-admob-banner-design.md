...
- **전역 하단 고정**: `MaterialApp`의 `builder` 파라미터로 모든 화면(로그인 화면 포함, 예외 없음)의 콘텐츠 아래에 배너를 고정한다. `showDialog`는 루트 `Navigator`(= `builder`의 `Expanded` 슬롯 안에 있는 `child`) 위로 푸시되므로, 다이얼로그는 이 `Expanded` 서브트리 안에 갇혀 배너와 겹치지 않는다 — 별도 오버레이 레이어라서가 아니다. 부수 효과로 다이얼로그의 모달 배리어(어두운 배경)도 배너를 덮지 않으므로, 다이얼로그가 떠 있는 동안에도 광고는 계속 보이고 탭 가능하다.
